namespace Before.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<ActionResult> Create(UserCreateViewModel model)
        {
            ...
            {
                await BlobCommandManager.CreateUserAsync(model.ToRequest()).ConfigureAwait(true);
        ...
        {
            var viewModel = await BlobQueryManager.GetUserDisableViewModelAsync(id).ConfigureAwait(true);
            if (viewModel == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Disable(UserDisableViewModel model)
        {
            if (ModelState.IsValid)
            {
                await BlobCommandManager.DisableUserAsync(model.ToRequest()).ConfigureAwait(true);
                return Json(new { success = true });
            }
        ...
        {
            var viewModel = await BlobQueryManager.GetUserUpdateViewModelAsync(id).ConfigureAwait(true);
            if (viewModel == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit(UserUpdateViewModel model)
        {
            if (ModelState.IsValid)
            {
                await BlobCommandManager.UpdateUserAsync(model.ToRequest()).ConfigureAwait(true);
                return Json(new { success = true });
            }
        ...
        {
            var viewModel = await BlobQueryManager.GetUserUpdatePasswordViewModelAsync(id).ConfigureAwait(true);
            if (viewModel == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> EditPassword(UserUpdatePasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        {
            var viewModel = await BlobQueryManager.GetUserEnableViewModelAsync(id).ConfigureAwait(true);
            if (viewModel == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Enable(UserEnableViewModel model)
        {
            if (ModelState.IsValid)
            {
                await BlobCommandManager.EnableUserAsync(model.ToRequest()).ConfigureAwait(true);
                return Json(new { success = true });
            }
            return PartialView("_EnableModal", model);
        }
        ...
        public ActionResult PageForCustomer(Guid id, int? page, int? pageSize)
        {
            ...
            var pageVm = AsyncHelpers.RunSync<UserPageViewModel>(() => BlobQueryManager.GetUserPageViewModelAsync(id, page.Value, pageSize.Value));
        ...
        {
            var viewModel = await BlobQueryManager.GetUserSingleViewModelAsync(id).ConfigureAwait(true);
            if (viewModel == null)
            {
                return HttpNotFound();
            }
