namespace Before.Controllers
{
    ...
    public class CustomerGroupController : Controller
    {
        ...
        public async Task<ActionResult> Create()
        {
            ...
            var viewModel = await BlobQueryManager.GetCustomerGroupCreateViewModelAsync(customerId).ConfigureAwait(true);
            if (viewModel == null)
            {
                return HttpNotFound();
            }
        ...
        public async Task<ActionResult> Create(CustomerGroupCreateViewModel model, params string[] selectedRoles)
        {
            if (ModelState.IsValid)
            {
                var result = await BlobCommandManager.CreateCustomerGroupAsync(model.ToRequest()).ConfigureAwait(true);
                if (result.Succeeded)
                {
                    ...
                    foreach (var role in selectedRoles)
                    {
                        ...
                        await BlobCommandManager.AddRoleToCustomerGroupAsync(new AddRoleToCustomerGroupRequest { GroupId = model.GroupId, RoleId = roleId });
                    }
                }
        ...
        {
            var viewModel = await BlobQueryManager.GetCustomerGroupDeleteViewModelAsync(id).ConfigureAwait(true);
            if (viewModel == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Delete(CustomerGroupDeleteViewModel model)
        {
            if (ModelState.IsValid)
            {
                await BlobCommandManager.DeleteCustomerGroupAsync(model.ToRequest()).ConfigureAwait(true);
                return Json(new { success = true });
            }
        ...
        {
            var viewModel = await BlobQueryManager.GetCustomerGroupUpdateViewModelAsync(id).ConfigureAwait(true);
            if (viewModel == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit(CustomerGroupUpdateViewModel model, params string[] selectedRoles)
        {
            if (ModelState.IsValid)
            {
                UpdateCustomerGroupRequest dto = model.ToRequest();
        ...
        public ActionResult PageForCustomer(Guid id, int? page, int? pageSize)
        {
            ...
            var pageVm = AsyncHelpers.RunSync<CustomerGroupPageViewModel>(() => BlobQueryManager.GetCustomerGroupPageViewModelAsync(id, page.Value, pageSize.Value));
        ...
        {
            var viewModel = await BlobQueryManager.GetCustomerGroupSingleViewModelAsync(id).ConfigureAwait(true);
            if (viewModel == null)
            {
                return HttpNotFound();
            }
